    ...
    for file_path in files:
        ...
        try:
            ...
            if cfg.output.vault_dir:
                ...
                cfg.output.vault_dir.mkdir(parents=True, exist_ok=True)
...
def setup(
    config_path: Annotated[
        Path | None,
        typer.Option("--config", "-c", help="Path to write config.toml"),
    ] = None,
) -> None:
    """Interactive setup wizard: create config.toml and optionally install macOS integrations."""
    dest = config_path or Path("config.toml")

    typer.echo("")
    typer.echo("drop2md setup wizard")
    typer.echo("=" * 40)
    typer.echo("")
    ...
    # ── Paths ────────────────────────────────────────────────────────────────
    typer.echo("── Paths ─────────────────────────────────")
    watch_dir = typer.prompt(
        "Watch directory (drop files here)",
        default="~/Documents/drop-to-md",
    )
    output_dir = typer.prompt(
        "Output directory (converted markdown goes here)",
        default="~/Documents/markdown-output",
    )
    vault_dir_raw = typer.prompt(
        "Obsidian vault directory (leave blank to skip)",
        default="",
    )
    vault_dir = vault_dir_raw.strip()

    typer.echo("")

    # ── AI Enhancement ───────────────────────────────────────────────────────
    typer.echo("── AI Enhancement ────────────────────────")
    typer.echo("Providers: none | ollama | claude | openai | gemini")
    provider = typer.prompt("AI provider", default="none").strip().lower()
    valid_providers = {"none", "ollama", "claude", "openai", "gemini", "hf"}
    ...
    if provider == "none":
        typer.echo("  AI enhancement disabled.")
        ai_enabled = False
    ...
    # ── Write config.toml ────────────────────────────────────────────────────
    vault_line = f'\nvault_dir = "{vault_dir}"' if vault_dir else ""
    vep_block = "\n\n[visual]\nenabled = true" if enable_vep else ""

    config_content = f"""\
# drop2md configuration — generated by `drop2md setup`
...
"""
    ...
    dest.write_text(config_content, encoding="utf-8")
    typer.echo(f"Config written to: {dest}")
    typer.echo("")

    # ── macOS integrations ───────────────────────────────────────────────────
    if sys.platform == "darwin":
        typer.echo("── macOS Integrations ────────────────────")
    ...
    # ── Connection test ──────────────────────────────────────────────────────
    if ai_enabled and provider == "ollama":
    ...
    typer.echo("")
    typer.echo("Setup complete. Run `drop2md check` to validate your configuration.")
...
def _render_status_panel(
    ...
    return Group(*lines)  # type: ignore[arg-type]
        ...
        try:
            with Live(console=console, refresh_per_second=4) as live:
                while True:
                    ...
                    live.update(_render_status_panel(cfg, config_path, pid))  # type: ignore[arg-type]
                    time.sleep(interval)
